...
(`feat/g13-optional-chain`). No GitHub issues were open. This is the
...
Closing G2 does **not** mean: default on-chain nonce/balance check, public TLS host, unsolicited volume, or "the local facilitator settles."
Optional local-facilitator chain checks exist when `VERITAS_FACILITATOR_CHAIN_CHECKS=1` and `VERITAS_RPC_URL` is set; they fail closed on RPC error. **G13 stays open** because the default path still does not consult the chain. Those stay on this list.
