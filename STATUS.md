...
| Catalog pull (`POST /v1/signals`) | Tested — Kalshi/Polymarket books stored latest-per-market; search miss is empty, not a dump |
| Unavailable vs empty catalog | Tested — a dead venue is not a miss and is not settled |
| URL observe (`POST /v1/notarize`) | Tested — same money path as catalog pull |
...
| Catalog honesty + CI quality gates | Working (`python -m veritas.evaluations.catalog`) |
...
| MCP (`veritas-mcp`; listed at `/v1/hooks`) | Tested against the SDK. Local free-mode catalog; no payment path |
...
| VCAE escrow (G12) | Library + HTTP lock/release/forfeit. GET never serves the signature. Forfeit re-runs `evaluate_challenge`. Release is loopback-only. Forfeit submit needs live facilitator. Not a vault. G2 closed (EIP-712 recover). Mainnet collect unproven. Catalog pull does not auto-attach a warranty. |
| Prediction-market catalog | Public Kalshi/Polymarket snapshots stored as evidence. Paid pull, free list/history. Arithmetic analysis. Prices, not verdicts. No trading. No research Q&A |
...
| Unsolicited demand | High | Zero unsolicited buyers. Catalog honesty proves invariants, not market fit |
...
library/HTTP primitives. Catalog pull does not auto-warrant. Mainnet collect
...
| Catalog honesty | Dead venue ≠ empty book; unavailable is not settled |
