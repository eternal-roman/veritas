...
> **Do this next: M7** (credits via SIWx; the last of Phase M), then **Phase
> N0** (the notary product). **O.8 is in review** — see the PR for
> `feat/o.8-supply-chain-hardening`; it is not on `main` until a human merges
> it.
>
> Follow-ons O.8 deliberately did **not** do, each a separate decision rather
> than an oversight: the **Dockerfile still installs from pyproject floors**
> (the image path is not hash-locked), the SBOM is **unsigned**, and it is
> uploaded as a workflow artifact rather than attached to the GitHub release —
> `gh release upload` needs `contents: write` on the one job that already holds
> the PyPI publishing identity, which is a maintainer's call, like the GHCR
> push.
>
...
> **O.8 is built and green, awaiting merge.** All 16 Action references are
> pinned to commit SHAs (each verified against the upstream tag via the GitHub
> API at pin time), dependencies are hash-pinned in `requirements.lock` (13
> packages) and `requirements-dev.lock` (115), and CI installs both with
> `--require-hashes`. The SBOM is generated by `cyclonedx-py` against the clean
> venv the wheel is installed into — the buyer's closure, not the CI
> toolchain — and the job fails if it disagrees with `requirements.lock`.
> `tests/test_supply_chain.py` is the witness (19 tests).
>
> The load-bearing detail: **pip evaluates environment markers against the
> running interpreter**, not the `--platform` you ask for. A lock generated on
> Windows gains `pywin32-ctypes` and *loses* `SecretStorage`/`jeepney`, which
> `twine`'s `keyring` needs only on Linux — so it installs fine locally and
> fails on CI against a package it has no hash for. Measured, not assumed: the
> same requirements resolved to 93 packages on Windows and 99 on Linux.
> `scripts/lock_requirements.py` therefore **refuses to generate off-target**
> rather than detecting the problem afterwards. Both locks were verified to
> install under `--require-hashes` on Linux/CPython 3.12 before commit.
>
> Also hardened: the package job installs the wheel over the hash-pinned
> runtime closure with `--no-deps` and runs `pip check`, so a drift between
> `pyproject` floors and `requirements.lock` fails in CI instead of at a
> buyer's install. Not claimed: the Docker image path is still unpinned, and
> the SBOM is unsigned.
>
